...
async def check_admin_privileges(
    ...
    if not membership:
        # Should be caught by get_current_tenant ideally, but safety check
...
async def invite_user(
    ...
    if not existing_user:
        ...
        target_user = await auth_service.create_user(user_create) # This creates a Personal Tenant for them too
...
async def update_user_role(
    ...
    # Prevention: Only Owner can change others to/from Owner? 
    # Or simplified: Admin can promote to Admin.
    if target_membership.role == "owner" and update_in.role != "owner":
        # Trying to demote owner
        if admin_membership.role != "owner":
             raise HTTPException(status_code=403, detail="Only Owner can demote Owner (after transfer).")
    
    target_membership.role = update_in.role
    await db.commit()
